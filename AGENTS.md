...
- `skills/<skill-name>/evals/evals.json` (recommended evaluation cases)
- `skills/<skill-name>/README.md` (required human-facing notes)
...
2. Ensure `SKILL.md` has valid YAML frontmatter with:
   - `name`
   - `description`
   - `metadata` containing `author` and `version`
...
5. Add or update eval cases in `evals/evals.json` when behavior changes materially.
...
- Update docs and eval assets (`README.md`, skill `README.md` when present, `evals/evals.json`) when behavior changes.
